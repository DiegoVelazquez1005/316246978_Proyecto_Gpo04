...
glm::vec3 encenderPointLight2 = glm::vec3(0.5, 0.5, 0.5);
...
int main(){
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "316246978_ProyectoTeoria_GPO04", nullptr, nullptr);
	...
	*/
	...
	//<<<<<<<<<<CUARTO2>>>>>>>>>>
	Model puertaSala((char*)"Models/puerta/puertaSala.obj");
	Model alfombra((char*)"Models/alfombra/alfombra.obj");
	Model fishLamp((char*)"Models/fishLamp/fishLamp.obj");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glm::vec3 pointLightActive2;
		pointLightActive = encenderPointLight;
		pointLightActive2 = encenderPointLight2;
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[3].ambient"), pointLightActive2.x, pointLightActive2.y, pointLightActive2.z);
		glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[3].diffuse"), pointLightActive2.x, pointLightActive2.y, pointLightActive2.z);
		...
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glUniform1i(glGetUniformLocation(lightingShader.Program, "activaTransparencia"), 0);
		puertaSala.Draw(lightingShader);
		//ALFOMBRA
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-6.7f, 0.01f, -2.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glUniform1i(glGetUniformLocation(lightingShader.Program, "activaTransparencia"), 0);
		alfombra.Draw(lightingShader);
		//FISHLAMP
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-5.0f, 1.375f, -5.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glUniform1i(glGetUniformLocation(lightingShader.Program, "activaTransparencia"), 0);
		fishLamp.Draw(lightingShader);
		//SILON
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-6.7f, 0.01f, -5.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glUniform1i(glGetUniformLocation(lightingShader.Program, "activaTransparencia"), 0);
		sillon.Draw(lightingShader);
		//MECEDORA
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-4.0f, 0.025f, -1.7f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glUniform1i(glGetUniformLocation(lightingShader.Program, "activaTransparencia"), 0);
		mecedora.Draw(lightingShader);
		//CUADRO
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-4.0f, 3.5f, -5.85f));
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	//valores de Spotlight para el cuarto 2
	if (keys[GLFW_KEY_9]) {
		if (active2 == false) {
			encenderPointLight2 = glm::vec3(0.5, 0.5, 0.5);
			active2 = true;
		}
		else {
			encenderPointLight2 = glm::vec3(0.0, 0.0, 0.0);
			active2 = false;
		}
	}
}
